...
async def research_model(
    ...
    """Call model for research queries to gather recent stock market news."""
    _LOGGER.info("Researching stock market news for section: %s", state.section.name)
    ...
    for count in range(_MAX_LLM_RETRIES):
        messages = [
            {"role": "system", "content": "/no_think You are a financial analyst researching recent market news. Generate specific search queries focused on recent S&P 500 and SPY news from the last 3 days."},
...
async def writing_model(
    ...
    """Call model to write the stock analysis section content."""
    _LOGGER.info("Writing stock analysis section: %s", state.section.name)
    ...
    for count in range(_MAX_LLM_RETRIES):
        ...
        if research_context:
            enhanced_prompt += f"\n\nAvailable Research Context (Recent Stock Market News):\n{research_context[:3000]}..."  # More context for financial analysis
        else:
            # For sections without research, emphasize writing from general knowledge
            enhanced_prompt += "\n\nNOTE: No specific research context is available. Write this section using your general knowledge about stock market analysis and investment principles. Provide actionable recommendations. Do not mention the lack of research or use placeholder text."

        messages = [
            {"role": "system", "content": "/no_think You are a financial analyst and investment advisor. Your task is to write actual investment analysis content immediately - not to plan, outline, or suggest what should be written. Write complete paragraphs with specific market data, news analysis, and actionable recommendations. Always provide clear BUY or WAIT guidance with detailed reasoning. Never use placeholder text like 'Note: content will be drafted' or similar phrases. Start writing the section content immediately."},
